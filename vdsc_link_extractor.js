                    ...
                    for (const url of REPORT_URLS) {
                        ...
                        try {
                            ...
                            const reports = await page.evaluate(() => {
                                ...
                                // Strategy 2: List items (existing)
                                const listItems = document.querySelectorAll('.list-news .item, .synthetic .item');
                                ...
                                // Strategy 3: Yellow Blocks (New)
                                // Selector: .list-report .item
                                const blockItems = document.querySelectorAll('.list-report .item');
                                blockItems.forEach(item => {
                                    // Date is split: h2 (day) + h4 (month - year)
                                    // Example: "06" + " tháng 11 - 2025 "
                                    const day = item.querySelector('.text-content h2.title')?.textContent?.trim();
                                    const monthYearRaw = item.querySelector('.text-content h4.title')?.textContent?.trim();

                                    let date = '';
                                    if (day && monthYearRaw) {
                                        // Clean up monthYearRaw: "tháng 11 - 2025" -> "11/2025"
                                        const parts = monthYearRaw.replace('tháng', '').split('-').map(s => s.trim());
                                        if (parts.length >= 2) {
                                            date = `${day}/${parts[0]}/${parts[1]}`; // 06/11/2025
                                        } else {
                                            date = `${day} ${monthYearRaw}`;
                                        }
                                    }

                                    const title = item.querySelector('.text-content h3')?.textContent?.trim();

                                    // The item itself is the anchor tag
